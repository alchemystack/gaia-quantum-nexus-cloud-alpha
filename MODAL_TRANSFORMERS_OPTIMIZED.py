...
if TYPE_CHECKING:
    ...
    torch = None  # Will be imported inside Modal container

# Create Modal app
app = modal.App("gaia-quantum-transformers-optimized")

# GPU configuration - A100 80GB for 120B model (UPDATED TO NEW SYNTAX)
gpu_config = "A100-80GB"  # Updated from modal.gpu.A100(count=1)

# PERSISTENT MODEL STORAGE VOLUME
# This volume persists across kernel resets and deployments
model_volume = modal.Volume.from_name("gaia-quantum-model-cache",
...
def download_model_if_needed():
    ...
    try:
        ...
        print(
            f"   Cache size: {sum(f.stat().st_size for f in cache_dir.rglob('*')) / 1e9:.1f} GB"
        )
...
def test_deployment():
    ...
    for profile in profiles:
        print(f"\n📊 Profile: {profile}")
        result = instance.generate.remote(prompt=test_prompt,
                                          max_tokens=50,
        ...
        if result["status"] == "success":
            ...
            if result.get("quantum_diagnostics"):
                ...
                print(
                    f"Avg modification: {diag.get('avg_logit_modification', 0):.4f}"
                )
        else:
